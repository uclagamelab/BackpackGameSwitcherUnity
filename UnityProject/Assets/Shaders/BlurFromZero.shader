...
Shader "Hidden/FastBlurFromZeroFujitsu" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_BlurRect("Rectangular blur area (Screen space)", Vector) = (0,0,1,1)
	...
	CGINCLUDE
		...
		bool inRectangle(float2 uv)
		{
		  return uv.x >= _BlurRect.x && uv.x <= _BlurRect.z
		  &&
		  uv.y >= _BlurRect.y && uv.y <= _BlurRect.w;
		}

		/*float rectangleProximity(float2 uv)
		{
			float xDiff = min(_BlurRect.x - uv.x, uv.x - _BlurRect.z);
		...
		{				
			fixed4 colorOrig = tex2D (_MainTex, i.uv);
			...
			color /= 4;
			return (1 - _BlurAmt) * colorOrig + _BlurAmt * color;
		}
		...
		half4 fragBlur8 ( v2f_withBlurCoords8 i ) : SV_Target
		{
			...
			half4 colorOrig =  tex2D(_MainTex, uv);
			half4 color = 0;

			bool inRect = inRectangle(uv);
			if (!inRect)
			{
				return colorOrig;
			}
